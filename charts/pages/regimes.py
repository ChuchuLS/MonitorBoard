"""
charts/pages/regimes.py
=======================
Section 03 — Curve Regimes (Phase 2: LIVE).

Classifies curve moves into directional regimes across nominal, real, and
inflation spread pairs. Uses 10D window by default.
"""
from __future__ import annotations
import pandas as pd, numpy as np
import plotly.graph_objects as go
from plotly.subplots import make_subplots
import streamlit as st
from config.pages import get_page
from config.i18n import (
    current_language, localized_curve_regime, localized_curve_type, tr,
)
from config.theme import section_color, BG, GRID, TEXT_DIM, DARK_LAYOUT
from config.tickers import TENOR_PAIRS
from charts.common import (
    render_page_header, render_kpi_strip,
    render_explanation_box, render_current_reading_box,
    render_model_note, render_section_footer,
)
from models.curve_regimes import (
    classify_pair_history, build_regime_matrix, dominant_regime,
    days_in_current_regime, REGIME_COLORS, REGIME_LABELS,
)
from ._context import PageContext


def _regime_ribbon(
    hist: pd.DataFrame, title: str, language: str, height: int = 200
):
    """Spread line + full-height regime colour ribbon."""
    h = hist.dropna(subset=["regime"])
    if h.empty:
        return None

    spread_bp = h["spread"] * 100
    y_min = float(spread_bp.min()) - 10
    y_max = float(spread_bp.max()) + 10

    fig = go.Figure()

    # Regime background — full-height colored bars
    for regime in REGIME_LABELS:
        mask = h["regime"] == regime
        if not mask.any():
            continue
        s = h.loc[mask]
        fig.add_trace(go.Bar(
            x=s.index, y=[y_max - y_min] * len(s), base=[y_min] * len(s),
            customdata=spread_bp.reindex(s.index).to_numpy(),
            marker=dict(color=REGIME_COLORS.get(regime, "#525252"), opacity=0.25,
                        line_width=0),
            name=localized_curve_regime(regime, language), showlegend=False,
            hovertemplate=(
                f"%{{x|%Y-%m-%d}}<br>{tr('Current state', '当前状态', language)}: "
                f"<b>{localized_curve_regime(regime, language)}</b>"
                f"<br>{tr('Spread', '利差', language)}: "
                "%{customdata:+.1f} bp<extra></extra>"
            ),
        ))

    # Spread line on top
    fig.add_trace(go.Scatter(x=h.index, y=spread_bp, mode="lines",
        line=dict(color="#fff", width=1.5),
        name=tr("Spread (bp)", "利差（bp）", language), showlegend=False))

    fig.update_layout(template="plotly_dark", paper_bgcolor=BG, plot_bgcolor=BG,
        font=dict(family="Inter, system-ui, sans-serif", size=10, color=TEXT_DIM),
        height=height, margin=dict(l=50, r=20, t=30, b=20), barmode="overlay",
        bargap=0, bargroupgap=0,
        title=dict(text=title, font=dict(size=12, color="#aaa"), x=0),
        yaxis=dict(title="bp", gridcolor=GRID, range=[y_min, y_max]),
        xaxis=dict(
            showgrid=False,
            tickformat="%Y-%m" if language == "zh" else None,
        ))
    return fig


def render(ctx: PageContext) -> None:
    language = current_language()
    page = get_page("regimes")
    from data.loader import latest_valid_date as _lvd
    from models.rate_decomposition import US_NOMINAL, US_BREAKEVEN
    _req = list(US_NOMINAL.values()) + list(US_BREAKEVEN.values())
    _ld = _lvd(ctx.df, _req) or ctx.df.index.max()
    latest = _ld.strftime("%b %d, %Y").upper()
    latest_display = _ld.strftime("%Y-%m-%d") if language == "zh" else latest
    render_page_header(
        page,
        latest_date=latest_display,
        viewing=tr(
            "Data source: DATA.xlsx / Sheet1",
            "数据来源：DATA.xlsx / Sheet1",
            language,
        ),
    )

    render_explanation_box(
        tr("Curve regime classification", "收益率曲线状态分类", language),
        tr(
            "Classifies each day's curve move into one of 7 regimes based on "
            "the direction of front and back yields and the spread change over "
            "a rolling window. Applied to <b>nominal, real, and inflation</b> "
            "curves across 6 tenor pairs.",
            "根据前端、后端收益率的变动方向和滚动窗口内的利差变化，将每日曲线变动分为七种状态。"
            "该方法应用于<b>名义、实际和通胀</b>曲线的六组期限组合。",
            language,
        ),
    )

    # Regime legend
    legend_bits = " &nbsp; ".join(
        f"<span style='display:inline-block;width:10px;height:10px;"
        f"background:{REGIME_COLORS[r]};vertical-align:middle;"
        f"margin-right:3px;border-radius:2px;'></span>"
        f"<span style='color:#aaa;font-size:10px;'>"
        f"{localized_curve_regime(r, language)}</span>"
        for r in REGIME_LABELS
    )
    st.markdown(f"<div style='margin:0.4rem 0 0.8rem;line-height:2;'>{legend_bits}</div>",
                unsafe_allow_html=True)

    # A. KPI strip — 2s10s regimes
    kpi_cards = []
    for ctype in ["Nominal", "Real", "Inflation"]:
        h = classify_pair_history(ctx.df, ctype.lower(), ("2Y", "10Y"), 10)
        if h.empty or h["regime"].dropna().empty:
            kpi_cards.append({
                "label": f"{localized_curve_type(ctype, language)} 2s10s",
                "value": "—",
            })
            continue
        regime = h["regime"].dropna().iloc[-1]
        days = days_in_current_regime(h["regime"])
        spread_now = h["spread"].dropna().iloc[-1] * 100 if h["spread"].dropna().shape[0] else np.nan
        kpi_cards.append({
            "label": f"{localized_curve_type(ctype, language)} 2s10s",
            "value": localized_curve_regime(regime, language),
            "sub": (
                f"{days} {tr('days', '天', language)} · {spread_now:+.0f} bp"
                if pd.notna(spread_now)
                else f"{days} {tr('days', '天', language)}"
            ),
            "accent": REGIME_COLORS.get(regime, "#525252"),
        })
    # Add 2s10s level
    h_nom = classify_pair_history(ctx.df, "nominal", ("2Y", "10Y"), 10)
    if not h_nom.empty and h_nom["spread"].dropna().shape[0]:
        lvl = h_nom["spread"].dropna().iloc[-1] * 100
        kpi_cards.append({
            "label": tr("2s10s Level", "2s10s 水平", language),
            "value": f"{lvl:+.0f} bp",
        })
    render_kpi_strip(kpi_cards)

    # B. 2s10s Regime History (6M lookback)
    st.markdown("<div style='margin:0.8rem 0 0.3rem;font-size:11px;color:#888;"
                "letter-spacing:0.1em;text-transform:uppercase;'>"
                f"{tr('2s10s regime history (6 months)', '2s10s 状态历史（6个月）', language)}</div>",
                unsafe_allow_html=True)

    for ctype in ["Nominal", "Real", "Inflation"]:
        h = classify_pair_history(ctx.df, ctype.lower(), ("2Y", "10Y"), 10)
        if h.empty:
            continue
        h_tail = h.iloc[-126:]  # ~6 months
        fig = _regime_ribbon(
            h_tail, f"{localized_curve_type(ctype, language)} 2s10s", language
        )
        if fig:
            st.plotly_chart(fig, use_container_width=True,
                            key=f"reg_{ctype.lower()}", config={"displayModeBar": False})

    # C. Regime Matrix
    st.markdown("<div style='margin:1rem 0 0.3rem;font-size:11px;color:#888;"
                "letter-spacing:0.1em;text-transform:uppercase;'>"
                f"{tr('Regime matrix (latest)', '状态矩阵（最新）', language)}</div>",
                unsafe_allow_html=True)

    matrix = build_regime_matrix(ctx.df, window=10)
    if not matrix.empty:
        def _color_cell(val):
            c = REGIME_COLORS.get(val, "#525252")
            return f"background-color: {c}22; color: {c}; font-weight: 700;"
        if language == "zh":
            display_matrix = matrix.map(localized_curve_regime)
            display_matrix.index = [
                localized_curve_type(value, language) for value in matrix.index
            ]
            localized_colors = {
                localized_curve_regime(key, language): color
                for key, color in REGIME_COLORS.items()
            }

            def _color_localized_cell(val):
                c = localized_colors.get(val, "#525252")
                return f"background-color: {c}22; color: {c}; font-weight: 700;"

            styled = display_matrix.style.map(_color_localized_cell)
        else:
            styled = matrix.style.map(_color_cell)
        st.dataframe(styled, use_container_width=True, height=160)

    # D. Regime Landscape
    st.markdown("<div style='margin:1rem 0 0.3rem;font-size:11px;color:#888;"
                "letter-spacing:0.1em;text-transform:uppercase;'>"
                f"{tr('Regime landscape', '状态概览', language)}</div>",
                unsafe_allow_html=True)
    if not matrix.empty:
        landscape_rows = []
        for ctype in matrix.index:
            d = dominant_regime(matrix.loc[ctype])
            landscape_rows.append({
                tr("Curve", "曲线", language): localized_curve_type(ctype, language),
                tr("Dominant", "主导状态", language): localized_curve_regime(d["regime"], language),
                tr("Count", "数量", language): d["count"],
                tr("Total", "合计", language): d["total"],
                tr("Divergent", "分歧组合", language): d["divergent"],
            })
        st.dataframe(pd.DataFrame(landscape_rows), hide_index=True, use_container_width=True)

    # E. Current Reading
    readings = []
    for ctype in ["Nominal", "Real", "Inflation"]:
        h = classify_pair_history(ctx.df, ctype.lower(), ("2Y", "10Y"), 10)
        if not h.empty and h["regime"].dropna().shape[0]:
            r = h["regime"].dropna().iloc[-1]
            readings.append(
                f"{localized_curve_type(ctype, language)} 2s10s："
                f"<b>{localized_curve_regime(r, language)}</b>"
            )
    if not matrix.empty:
        d = dominant_regime(matrix.loc["Nominal"])
        readings.append(
            f"{tr('Nominal dominant', '名义曲线主导状态', language)}："
            f"<b>{localized_curve_regime(d['regime'], language)}</b> "
            f"({d['count']}/{d['total']} {tr('pairs', '组', language)})"
        )
        readings.append(
            f"{tr('Divergent pairs', '分歧组合', language)}："
            f"<b>{d['divergent']}</b>"
        )
    if readings:
        render_current_reading_box(
            tr("Current reading", "当前读数", language), "<br>".join(readings)
        )

    # F. Methodology
    render_model_note(
        tr("Methodology", "方法说明", language),
        tr(
            "Uses a <b>10-day regime window</b> and available 2Y / 5Y / 10Y / 30Y "
            "tenors (6 pairs). 1Y-based pairs are skipped until 1Y data is added. "
            "Neutral threshold: |spread change| &lt; 1.5 bp.",
            "使用<b>10日状态窗口</b>和可用的 2Y / 5Y / 10Y / 30Y 期限（共六组）。"
            "在加入1年期数据前，涉及1Y的组合保持不计算。"
            "中性阈值：|利差变化| &lt; 1.5 bp。",
            language,
        ),
    )

    from charts.common import render_data_source_note
    render_data_source_note("DATA.xlsx / Sheet1", latest_display)
    render_section_footer(page)
