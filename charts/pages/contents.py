...
from config.i18n import (
    LANG_ZH, STATUS_ZH, current_language, localized_page, localized_regime, tr,
)
...
def render(ctx: PageContext) -> None:
    ...
    st.markdown("<div style='margin:0.4rem 0 0.3rem;font-size:11px;color:#888;"
                ...
                + tr("Today's dashboard state", "今日看板状态", language)
    ...
    readings = [(tr("Latest valid data date", "最新有效数据日期", language), latest)]
    ...
    if pd.notna(r.latest):
        ...
        readings.append((tr("Composite Liquidity Index — Official", "综合流动性指数 — 正式值", language),
                         f"<span style='color:{regime_color};'>{r.latest:.1f} "
                         f"({localized_regime(regime, language)}){official_suffix}</span>"))
        if r.preliminary_date is not None and pd.notna(r.preliminary_latest):
            ...
            readings.append((
                tr("Composite Liquidity Index — Preliminary", "综合流动性指数 — 初步值", language),
                f"{r.preliminary_latest:.1f} · {r.preliminary_date.date()} · "
                f"{prelim_buckets}/{HEADLINE_REQUIRED_BUCKETS} {tr('buckets', '板块', language)} · "
                f"{prelim_components} {tr('components', '组件', language)}",
            ))
        for h in ("1w", "1m", "3m"):
            v = changes.get(h)
            if v is not None and pd.notna(v):
                readings.append((tr(f"CLI {h.upper()} change", f"CLI {h.upper()} 变化", language),
                                 f"{v:+.1f} {tr('pts', '点', language)}"))
    ...
    render_current_reading_list(tr("Today's readings", "今日读数", language), readings)
    if state_failures:
        st.warning(
            tr("Some dashboard-state summaries are unavailable: ",
               "部分看板摘要暂不可用：", language)
            + "; ".join(state_failures)
            + tr(". No missing reading was replaced with zero or a proxy.",
                 "。任何缺失读数均未以零值或代理值替代。", language)
        )
    ...
    try:
        ...
        st.markdown(
            ...
            "letter-spacing:0.1em;text-transform:uppercase;'>"
            f"{tr('Content coverage vs reference PDF', '相对参考 PDF 的内容覆盖', language)}</div>", unsafe_allow_html=True)
        cov_items = [
        ...
        render_current_reading_list(tr("Model coverage", "模型覆盖", language), cov_items)
        st.caption(tr("See 08 · Model Roadmap for the full content gap analysis.",
                      "完整内容缺口分析请见 09 · 模型路线图。", language))
    except Exception as exc:
    ...
    st.markdown("<div style='margin:1.2rem 0 0.4rem;font-size:11px;color:#888;"
                "letter-spacing:0.1em;text-transform:uppercase;'>"
                f"{tr('Sections', '页面目录', language)}</div>", unsafe_allow_html=True)
    rows_html = []
    for p in PAGES:
        color = section_color(p["color_key"])
        status = p["status"]
        status_label = STATUS_ZH.get(status, STATUS_LABELS[status]) if language == LANG_ZH else STATUS_LABELS[status]
        status_color = STATUS_COLORS[status]
        display_page = localized_page(p, language)
        title = _html.escape(display_page["title"])
        desc = _html.escape(display_page["description"])
    ...
    legend_bits = " &nbsp; ".join(
        ...
        f"text-transform:uppercase;'>{STATUS_ZH.get(k, STATUS_LABELS[k]) if language == LANG_ZH else STATUS_LABELS[k]}</span>"
    ...
    st.markdown(f"<div style='margin-top:10px;font-size:11px;'>"
                ...
                f"{tr('Legend', '图例', language)}</span> &nbsp;{legend_bits}</div>", unsafe_allow_html=True)

    # ── Export download (lazy) ──
    with st.expander(tr("Export research pack", "导出研究报告", language), expanded=False):
        st.caption(tr(
            "Download the complete linked Board as a 16:9 English PDF. The file contains the cover, contents, all registered pages, PDF bookmarks and internal navigation.",
            "下载完整的 16:9 英文版 PDF，包含封面、目录、全部页面、书签和内部导航。",
            language,
        ))
        if ctx.pdf_export_builder:
            try:
                st.download_button(
                    tr("⬇  Download complete English Board PDF", "⬇  下载完整英文版 PDF", language),
